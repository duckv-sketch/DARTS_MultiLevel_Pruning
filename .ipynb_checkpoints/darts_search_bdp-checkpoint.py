...
#     error_history_train = ErrorHistory(prune_every)
#     error_history_val = ErrorHistory(prune_every)
...
#             x_train, x_val = x_train.squeeze(-1), x_val.squeeze(-1)

#             optimizer_alpha.zero_grad()
#             logits_val = model(x_val)
#             loss_alpha = criterion(logits_val, y_val)
#             entropy_reg = -torch.sum(torch.softmax(model.alphas_normal, dim=-1) * torch.log_softmax(model.alphas_normal, dim=-1))
#             loss_alpha += 0.001 * entropy_reg
#             loss_alpha.backward()
...
#             all_targets_train.extend(y_train.cpu().numpy())

#             sample_ids = range(step * y_train.size(0), (step + 1) * y_train.size(0))
#             err_train, _ = compute_errors(model, [(x_train, y_train)], device)
#             err_val, _ = compute_errors(model, [(x_val, y_val)], device)
#             error_history_train.update(sample_ids, err_train)
#             error_history_val.update(sample_ids, err_val)
...
#         print(f"Precision: {val_precision:.4f} | Recall: {val_recall:.4f} | F1: {val_f1:.4f} | Time: {duration:.2f}s")
...
#                 'epoch': epoch + 1,
#                 'val_acc': val_acc,
#                 'genotype': None  # updated later
#             }
#             print(f"[Checkpoint] New best model saved at epoch {epoch + 1} with Val Acc = {val_acc:.4f}")

#         scheduler.step()

#         if (epoch + 1) % prune_every == 0 and epoch <21:
#             T_current = 1.5 * (0.95 ** epoch)
#             prune_operators(model, temperature=T_current, mu=0.5)
#             print(f"[Annealable Pruning] Applied at Epoch {epoch + 1} with T = {T_current:.4f}")

#             voe_train = error_history_train.compute_voe()
#             voe_val = error_history_val.compute_voe()

#             n_train_prune = int(len(voe_train) * pt)
#             n_val_prune = int(len(voe_val) * pv)

#             voe_train_sorted = sorted(voe_train.items(), key=lambda x: x[1])
#             voe_val_sorted = sorted(voe_val.items(), key=lambda x: -x[1])

#             prune_T = set(k for k, _ in voe_train_sorted[:n_train_prune])
#             prune_V = set(k for k, _ in voe_val_sorted[:n_val_prune])

#             all_T = set(range(len(train_loader.dataset.y)))
#             all_V = set(range(len(val_loader.dataset.y)))

#             keep_T = list(all_T - prune_T)
#             keep_V = list(all_V - prune_V)
...
#             print(f"                     Remaining train: {len(keep_T)} | val: {len(keep_V)}")

#     if best_model_state is not None:
#         torch.save(best_model_state, "best_model20.pth")
#         print(f"\nBest model saved to 'best_model20.pth' with Val Acc = {best_val_acc:.4f}")
#         model.load_state_dict(best_model_state)

#     genotype_normal = parse_genotype(model.alphas_normal.detach().cpu().numpy(), steps=5)
#     genotype_reduce = parse_genotype(model.alphas_reduce.detach().cpu().numpy(), steps=5)
#     final_genotype = Genotype(
#         normal=genotype_normal,
#         normal_concat=list(range(2, 5)),
#         reduce=genotype_reduce,
#         reduce_concat=list(range(2, 5))
#     )

#     best_config['genotype'] = str(final_genotype)

#     with open("searched_genotype20.txt", "w") as f:
#         f.write(str(final_genotype))
...
#     return final_genotype, train_loader, val_loader
import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
...
def parse_genotype(alpha, steps=7, is_reduce=False):
    gene = []
    start = 0
    ...
    # --- Ép ít nhất 7 phép toán conv trong reduction cell ---
    if is_reduce:
        conv_ops = ['conv_1x1', 'conv_3x3', 'sep_conv_1x3', 'sep_conv_1x5', 'dil_conv_1x3', 'dil_conv_1x5']
        conv_count = sum(1 for op, _ in gene if op in conv_ops)

        if conv_count < 7:
            for idx, (op, inp) in enumerate(gene):
                if op in ['avg_pool_3x3', 'max_pool_3x3', 'skip_connect', 'none']:
                    for alt in conv_ops:
                        if alt in PRIMITIVES:
                            gene[idx] = (alt, inp)
                            conv_count += 1
                            break
                if conv_count >= 7:
...
def train_darts_search_bdp(train_loader, val_loader, num_classes, epochs=20, device='cuda', prune_every=5, pt=0.15, pv=0.05):
    ...
    for epoch in range(epochs):
        ...
        genotype_normal = parse_genotype(model.alphas_normal.detach().cpu().numpy(), steps=7)
        genotype_reduce = parse_genotype(model.alphas_reduce.detach().cpu().numpy(), steps=7, is_reduce=True)
        current_genotype = Genotype(
            normal=genotype_normal,
            normal_concat=list(range(2, 7)),
            reduce=genotype_reduce,
            reduce_concat=list(range(2, 7))
        )
        ...
        if (epoch + 1) % prune_every == 0 and epoch < 30:
            T_current = 1.5 * (0.95 ** epoch)
            prune_operators(model, temperature=T_current, mu=0.5)
